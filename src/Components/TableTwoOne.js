  ...
  const processData = (data, fileName) => {
    ...
    if (getFileExtension(fileName) == "xlsx") {
      ...
      let newData = trimmedSheetData.map((item) => {
        ...
        return {
          ...item,
          "Hours worked": isNaN(getHoursBetween(item["Start Time"], item["Finish Time"])) ? 0 : getHoursBetween(item["Start Time"], item["Finish Time"]),
          "Active status": (item["Onboard Total"] > 1) ? "Active" : "Inactive",
          "Typical Payment Time": paymentTime !== null && paymentTime !== "" ? paymentTime : "Not OSH", // Check for empty string or null
        };
      });
      ...
      newData = newData.map((item) => {
        ...
        const oshCost = isActive && isParcelRate
...
return(
  <div style={{ width: "100%", overflowX: "auto",overflow: "visible" }}>
    <table class="table-auto border-x border-b w-full text-left text-gray-800" style={{ tableLayout: "fixed" }}>
          <thead className="">
            <tr>
